...
// In a production setup you might want to move these to environment variables,
// but for this standalone HTML app we keep them inline for simplicity.
const SUPABASE_URL = "https://qesxymvwgokbwbjizxtw.supabase.co";
const SUPABASE_ANON_KEY = "sb_publishable_Pt5NiMLM9HBGwsWEXXTpJA_QDm2Nllh";
...
// --- Supabase helpers (auth + scoring / leaderboard) ---
...
async function supabaseSignInWithEmailPassword(email, password, isSignUp) {
  if (!supabaseClient) return { user: null, error: new Error("Supabase not initialised") };
  try {
    if (isSignUp) {
...
// Save result for the current authenticated user into the `profiles` table.
async function saveResult(pointsEarned) {
  if (!supabaseClient) return;
  ...
  if (userError || !user) {
    console.warn("No logged-in user; skipping saveResult.");
    return;
  }
  ...
  let prevLastPlayed = null;

  if (!selectError && existing) {
    prevTotalPoints = existing.total_points || 0;
    prevLastPlayed = existing.last_played_date;
    ...
    if (prevLastPlayed === today) {
      newStreak = existing.streak || 1; // already counted today
    } else if (prevLastPlayed === yesterdayStr) {
      newStreak = (existing.streak || 0) + 1; // continue streak
    } else {
      newStreak = 1; // reset streak
    }
  }

  const username = existing?.username || user.email || "Anonymous";
  ...
  const upsertPayload = {
    ...
    total_points: prevTotalPoints + (pointsEarned || 0),
  };
  ...
  if (upsertError) {
    console.error("Failed to save result:", upsertError.message || upsertError);
  }
}
...
async function getLeaderboard(limit = 50) {
  ...
  const { data, error } = await supabaseClient
    .from("profiles")
    .select("username, streak, total_points")
...
function startSequenceTimer() {
  if (typeof document === "undefined") return;
  if (sequenceTimerInterval != null) {
    clearInterval(sequenceTimerInterval);
    sequenceTimerInterval = null;
  }
  state.sequenceStartTime = Date.now();
  state.sequenceTotalTimeSec = 0;
  sequenceTimerInterval = setInterval(() => {
    if (!state.sequenceStartTime) return;
    const now = Date.now();
    const secs = Math.max(0, Math.floor((now - state.sequenceStartTime) / 1000));
    state.sequenceTotalTimeSec = secs;
    const nodes = document.querySelectorAll(".sequence-timer-value");
    nodes.forEach((el) => {
...
function showSequenceCountdown(onDone) {
  ...
  timeRow.className = "sequence-countdown-time";
  const label = document.createElement("span");
  label.textContent = "Time";
  const value = document.createElement("span");
  value.className = "sequence-timer-value";
  const secs = Math.max(0, state.sequenceTotalTimeSec || 0);
  value.textContent = `${secs}s`;
...
const state = {
  ...
  sequenceStartTime: null, // ms timestamp when entering this sequence
  sequenceTotalTimeSec: 0, // total elapsed seconds for this playthrough
...
function renderEndScreen() {
  ...
  if (supabaseClient) {
    // Do not block the UI; log any errors in the background
    saveResult(state.score).catch((e) => {
      console.error("saveResult failed:", e);
    });
  }

  // Points vs time division animation
  const timeSec = Math.max(1, state.sequenceTotalTimeSec || 0);
  const rawPoints = state.score || 0;
  const stepsCount = Array.isArray(puzzleSequence.steps) ? puzzleSequence.steps.length : 0;
  const avgTimePerRiddle =
    stepsCount > 0 ? timeSec / stepsCount : timeSec;
  const avgTimeRounded = Math.max(1, Math.round(avgTimePerRiddle));
  const efficiency = Math.round(rawPoints / avgTimeRounded);
  ...
  line1.textContent = `You scored ${rawPoints} points.`;

  const line2 = document.createElement("div");
  line2.className = "division-line division-line-2";
  line2.textContent = `It took you ${timeSec} seconds in total.`;

  const line3 = document.createElement("div");
  line3.className = "division-line division-line-3";
  line3.textContent = `On average, each riddle took about ${avgTimeRounded} seconds, so your final score is ${rawPoints}/${avgTimeRounded} ≈ ${efficiency} points.`;
...
function renderLibrary() {
  ...
  // Supabase auth + leaderboard block
  const supabaseSection = document.createElement("div");
  supabaseSection.className = "supabase-section";

  const authBox = document.createElement("div");
  authBox.className = "supabase-auth";

  const authTitle = document.createElement("div");
  authTitle.className = "field-label";
  authTitle.textContent = "Strike login (Supabase)";
  ...
  authRow.appendChild(emailInput);
  ...
  authButtons.className = "buttons-right auth-buttons";
  ...
  signUpBtn.addEventListener("click", async () => {
    ...
    const { user, error } = await supabaseSignInWithEmailPassword(email, password, true);
    if (error) {
  ...
  signInBtn.addEventListener("click", async () => {
    ...
    const { user, error } = await supabaseSignInWithEmailPassword(email, password, false);
    if (error) {
  ...
  authBox.appendChild(authButtons);

  const authStatus = document.createElement("div");
  authStatus.className = "muted auth-status";
  if (state.currentUser && state.currentUser.email) {
    authStatus.textContent = `Logged in as ${state.currentUser.email}`;
  } else {
    authStatus.textContent = "Not logged in – scores will not be saved to the online leaderboard.";
  }
  authBox.appendChild(authStatus);

  supabaseSection.appendChild(authBox);

  const leaderboardBox = document.createElement("div");
  leaderboardBox.className = "leaderboard-box";
  const leaderboardTitle = document.createElement("div");
  leaderboardTitle.className = "field-label";
  leaderboardTitle.textContent = "Strike leaderboard (top players)";
  ...
  supabaseSection.appendChild(leaderboardBox);
  content.appendChild(supabaseSection);
    ...
    getLeaderboard(20)
      .then((rows) => {
        ...
        rows.forEach((row, idx) => {
          ...
          li.textContent = `#${idx + 1} ${row.username || "Unnamed"} – streak: ${
            row.streak ?? 0
          }, total points: ${row.total_points ?? 0}`;
  ...
  let overallPoints = 0;
  let overallTime = 0;
  let overallRiddles = 0;
  sequenceLibrary.forEach((seq) => {
    overallPoints += seq.statsPointsAccum || 0;
    overallTime += seq.statsTimeAccumSec || 0;
    overallRiddles += seq.statsRiddlesAccum || 0;
  });

  const avgTimePerRiddle =
    overallRiddles > 0 ? Math.round(overallTime / overallRiddles) : 0;
  ...
  overall.textContent =
    overallRiddles > 0
      ? `Overall: ${overallPoints} points collected • average ${avgTimePerRiddle}s per riddle`
      : "Overall: no results yet – play a sequence to see total points and average time per riddle.";
  ...
  sequenceLibrary.forEach((seq, idx) => {
    ...
    const totalTime = seq.statsTimeAccumSec || 0;
    const totalRiddles = seq.statsRiddlesAccum || 0;
    const avgTimePerRiddle =
      totalRiddles > 0 ? Math.round(totalTime / totalRiddles) : 0;

    const statsText =
      runs > 0
        ? ` • Played ${runs} time${runs === 1 ? "" : "s"} • ${totalPts} pts total • avg ${avgTimePerRiddle}s / riddle`
        : "";
...
function createCardShell({ badgeText, badgeSecondary, title, subtitle, progressText }) {
  ...
  if (state.screen === "sequence") {
    ...
    if (state.mode === "play") {
      ...
      timerWrap.className = "sequence-timer";
      const timerLabel = document.createElement("span");
      timerLabel.className = "sequence-timer-label";
      timerLabel.textContent = "Time";
      const timerValue = document.createElement("span");
      timerValue.className = "sequence-timer-value";
      const secs = Math.max(0, state.sequenceTotalTimeSec || 0);
